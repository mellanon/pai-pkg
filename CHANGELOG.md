...
- **Installing a composition: `references[]` resolution and ONE combined capability review** ([#400](https://github.com/the-metafactory/arc/issues/400), design `docs/design-factory-type.md` D2/D4/D5). `arc install <factory>` is now one command and one decision. A `bundle`/`factory` manifest may declare `references[]` (its members, each at an **exact** version), and a `factory` may additionally declare `tools:` (host binaries) and `produces:` (the capability the composition provides). Install validates the manifest, checks the declared tools against the host, resolves every reference and reads every member manifest, computes the union of the members' capability surfaces, asks **one** confirmation, and only then installs the members through the ordinary `install()` path. The resolved membership and its pins are recorded and readable via `arc list --json` (`.packages[].composition.members`).

  Refusals fire **before any member lands**, because resolution and installation are separate phases: a **range version** in `references[]` (`>=6.0.0`, `^1.2.0`, `1.x`, `latest`) — a factory release is a reproducible snapshot, so the pin must be exact, and this is checked at install and not only at publish (D4); a **missing tool**, named, before a single byte is fetched; a **member manifest that fails validation** or cannot be resolved, which aborts the *whole* install so a good member never lands beside a bad one (D2's honesty rule); a **member that is itself a composition**; and a **declined review**. A member whose own manifest version disagrees with the pin it was fetched at is refused too — a broken snapshot, however it came about.

  **Compositions do not nest**, and that is a consent rule rather than a layering preference. A `bundle`/`factory` member declares no capabilities of its own, so it contributes *zero* rows to the union: the combined review would render "Risk: LOW / (none)" — a truthful summary of that member's manifest and a completely false summary of what installing it does — and the member installer would then install the nested composition's own members with no review at all. Flatten the members into the outer `references[]` instead. arc refuses what the registry refuses here (`BUNDLE_RECURSIVE`, meta-factory PR #574 §8), so the ecosystem has one answer.

  **Consent is bound to the bytes, not to a label.** A `repo:` member is resolved by finding the tag for its pinned version and then pinning the member install to that **commit** — a tag is mutable, consent is read from one clone and the member lands from another, and `git tag -f` in between would otherwise install code nobody reviewed. Independently of route, each member's **recorded** capability surface is checked back against the reviewed one after it lands; a difference refuses the install and reports what appeared or vanished. A member that was *already* installed and whose recorded surface differs is warned about rather than refused — that install was approved separately and this composition did not put it there.

  **An interrupted composition install is visible.** The composition record is opened *before* the first member lands and closed after the composition's own row commits, so a kill or a member failing at runtime leaves a `pending` record naming exactly which members landed — instead of member packages that look standalone and no trace that a factory was attempted. `arc list` prints these under **Incomplete compositions**, `arc list --json` exposes them in a top-level `compositions` array (an interrupted composition has no package row, so `packages[]` cannot represent it), and the error message from a failed member names the already-landed ones so the operator knows what to clean up.

  **A composition declares no `capabilities:` block.** arc#399 made the block optional for `bundle`/`factory`; declaring one is now an error, on the same reasoning taken one step further — a composition ships no code, so a declared block creates two answers to "what can this do": the one an author reads in the repo, and the computed union that actually governs. Matches the registry's own gate.

  The **exact-pin grammar mirrors the registry's stored-version grammar byte for byte** (`/^\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?$/`), and is exported so the publish side ([#402](https://github.com/the-metafactory/arc/issues/402)) shares the constant rather than copying it. Two consequences point in opposite directions and both are deliberate: build metadata (`1.2.3+build.5`) and a hyphen inside a prerelease (`1.2.3-rc-1`) are now refused, because the registry cannot store either and such a pin could never resolve; leading zeros (`1.02.3`) are tolerated even though SemVer 2.0.0 forbids them, because the registry *can* store them and refusing would be a false refusal against a legitimately published version.

  The combined review shows the **full union**, nothing summarised away: it replaces the per-member prompts, so anything it omitted would be something nobody is ever asked about. Filesystem paths and network hosts are deduped and attributed to the member(s) that asked for them, secrets likewise; `bash.allowed` is OR'd and `restricted_to` unioned, and **one** member with unrestricted bash marks the whole composition unrestricted and is flagged by name. Risk is computed over the union rather than per member, so two members that are each merely `medium` — one with network, one with filesystem write — surface as the `high` download-and-write composition they actually are. Under `--yes` the approved surface is still written to stderr, so a CI log shows what was approved.

  Two things warn rather than refuse. A factory declaring a higher `tier:` than the **MIN of its members'** gets an `arc audit`-style warning naming the member that pulled it down (trust never averages up; the tier is re-checked at install because a member's tier can change by revocation — D5). A tool whose `--version` output arc cannot parse warns and proceeds, the same fail-open posture `semver.ts` documents for its advisory checks — a false refusal on a security gate teaches operators to reach for `--yes`. A composition declaring no `references[]` installs exactly as it did before.

- **`type: bundle` and `type: factory` — the reference-composition types** ([#399](https://github.com/the-metafactory/arc/issues/399), design `docs/design-factory-type.md` D7.1–D7.2). Both now validate at install and at publish. A composition packages other packages rather than an artifact payload of its own, so install plans **no** per-type symlink for them — `provides.files` is still honored by the type-agnostic pass, and a missing `capabilities:` block is no longer an error (a composition has nothing of its own to declare; its surface is the union of its members', computed at install once reference resolution lands — D2). Resolving `references[]` and actually installing members is the next slice; until it lands a bundle/factory install is a manifest-only no-op rather than the `Unsupported artifact type` throw it would have been.

  The two meanings of "bundle" both survive and are now written down where each is used (D1): `metafactory-bundle-<name>` remains a **repo-name class** whose members declare ordinary installable types, while `type: bundle` is the **manifest type** (registry DD-111, migration `0012_add_bundle_type.sql`). `factory` sidesteps the collision by not reusing the word. The README gains an **Artifact types** section documenting all 14 types plus a side-by-side table for the two meanings; the plugin-bundles section's claim that "there's no separate 'bundle' artifact type" is corrected — true when written, not any more.

  `arc validate` and `arc install` now agree on a minimal composition manifest. Strict validation exempts `bundle`/`factory` from the arc#240 capabilities-presence rule for the same reason the loader does, so the validator no longer rejects exactly what install accepts. The exemption is presence-only and extends to no other type: a composition that *does* declare `capabilities:` has it validated in full, and every type with a surface of its own still must declare one.

### Fixed

- **Three drifted copies of the artifact-type enum collapse into one, and `system` / `process` / `governance` become publishable** ([#397](https://github.com/the-metafactory/arc/issues/397), [#399](https://github.com/the-metafactory/arc/issues/399)). arc hand-copied its type list into three places — `ArcManifest["type"]`, the strict validator's `VALID_TYPES`, and the publish validator's `VALID_TYPES` — and they had already diverged: the publish copy was missing `system`, `process` and `governance`, so a package of one of those types installed fine and then failed `arc publish` with "type … is not a recognized artifact type". An author could install what they could not ship.

  All three sites (plus `arc list --type` and `arc search --type`, which had drifted their own way and rejected `--type library` / `--type process` for packages arc will happily list) now derive from a single `ARTIFACT_TYPES` array, so the parity that `test/unit/type-set-parity.test.ts` used to police is true by construction. The test remains as the regression guard: it fails the moment a second literal list reappears. A registry `bundle`/`factory` package also keeps its own type in search results now, instead of being folded to `skill` by the inbound type map's fallback.

- **`--pin` on an already-installed package moves the checkout instead of being silently ignored** ([#396](https://github.com/the-metafactory/arc/issues/396)). `arc install <repo> --pin <sha>` on an installed package exited 0, printed success, and left the checkout on whatever commit it was already on: the duplicate guard returned `alreadyInstalled` before any git ran, and `checkoutPinnedRef` sat inside the clone-only branch, unreachable. A pinned install is a determinism claim, so exit 0 + wrong commit is the worst available answer — automation trusts it (found while building smithy's `metafactory_cortex` role for crucible AC-3). The re-run now fetches `--force --tags`, resolves the pin to a commit (remote-tracking ref first), and checks it out through the same `checkoutPinnedRef` the fresh-clone path uses; a branch pin fast-forwards onto the fetched tip. Already at the pinned ref → no-op success, nothing touched. A re-run *without* `--pin` is unchanged.

  Three ways a re-pin can still hand back a wrong-but-successful result are closed with it: a **moved tag** (a plain `--tags` fetch will not update an existing local tag, so a re-tagged `v1.0.0` resolved to the stale commit), an **advanced branch** (the bare local branch name resolved to the last checkout, so an origin that moved was reported as "nothing to do"), and a **basename collision** (the repo-name duplicate guard matched `repo_url.endsWith(<name>)`, so a pin named against repo B could move repo A's checkout — now refused, naming both URLs).

  A move refuses loudly and changes nothing when the checkout has uncommitted changes (`node_modules/`, `bun.lock` and `.DS_Store` are arc's own leavings and don't count — that filter is now shared with `arc verify` instead of duplicated), when the local branch has diverged from origin (fast-forwarding would discard local commits), or when the ref **widens the package's declared capabilities** without consent — the widened surface is shown and confirmed before anything moves, and a non-interactive run without `--yes` is refused rather than inheriting the old approval. On a successful move arc installs the new ref's node dependencies (a genuine failure rolls the checkout back), then records the new version *and* re-records the capability surface, so `arc audit` describes the code actually checked out. Lifecycle scripts, hooks, templates, and extensions are still `arc upgrade`'s job. `--pin` values are also validated inside `install()` now, not only at the CLI boundary.

  Two consequences of the resolver worth naming. A ref name that is **both a tag and a branch** pointing at different commits is refused: `git checkout <name>` prefers the tag while the re-pin resolver prefers the branch, so the checks would validate one commit and another would land — including the capability gate, which is a consent bypass, not a curiosity. The landing now targets the resolved SHA directly (a branch pin lands on the branch at that SHA) and a post-checkout mismatch is refused rather than recorded. Separately, **unrestricted bash is now a recorded capability**: `capabilities.bash.allowed` with no `restricted_to` writes a `bash: (unrestricted)` row where it previously wrote nothing at all — so the recorded surface no longer says "no bash" for a package with unrestricted bash, and dropping a `restricted_to` list reads as a widening instead of a narrowing. Fresh installs, upgrades, and re-pins all emit the shape, so an existing install records the new row the next time any of them runs; `arc audit`'s bash count rises accordingly for affected packages.
